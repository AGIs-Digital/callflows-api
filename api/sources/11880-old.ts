...
export async function search11880(query: string): Promise<SourceResult> {
  ...
  try {
    ...
    if (isLocal) {
      ...
      // FALLBACK: Verwende eine API-basierte Lösung statt Browser-WS
      if (!wsEndpoint) {
        console.log('🔧 No remote browser configured, switching to API-only approach for 11880');
        // Direkt zu lokalen Fallbacks springen, da kostenlose WS-Services unzuverlässig sind
